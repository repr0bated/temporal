...
func (ch *commandHandler) handleScheduleCommand(
	...
	// AddHistoryEvent creates the event and calls ScheduledEventDefinition.Apply,
	// which creates the Operation component and adds it to the workflow.
	_, err := wf.AddHistoryEvent(chasmCtx, enumspb.EVENT_TYPE_NEXUS_OPERATION_SCHEDULED, func(he *historypb.HistoryEvent) {
...
func (ch *commandHandler) handleCancelCommand(
	...
	// AddHistoryEvent calls CancelRequestedEventDefinition.Apply, which calls op.Cancel().
	_, err := wf.AddHistoryEvent(chasmCtx, enumspb.EVENT_TYPE_NEXUS_OPERATION_CANCEL_REQUESTED, func(he *historypb.HistoryEvent) {
